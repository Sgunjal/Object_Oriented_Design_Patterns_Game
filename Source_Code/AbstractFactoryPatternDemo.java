...
public class Animal{   
   ...
   public void two_legs_animal(String animalname)                                         // Init method where we can initialize all obejcts and set background color.
	...
	public static void main(String args[])
	{
		...
		Animal afpd=new Animal();
		
		System.out.println("");
		System.out.println("Animal Name who has two legs !!!!");
		
		for(int i=0;i<=2;i++)
		{
			afpd.two_legs_animal(two_leg_animalnames[i]);
		}
		
		System.out.println("");
		System.out.println("Animal Name who has four legs !!!!");
		
		for(int i=0;i<=2;i++)
		{
			afpd.four_legs_animal(four_leg_animalnames[i]);
		}		
	}
}